...
export const phoneCountryCodes: PhoneCountryCode[] = dialCodes.map(
  ({ code, dialCode }) => ({
    code,
    dialCode,
    name: fallbackNames[code] || countryNameByCode.get(code) || code,
  })
);

export const phoneCountryCodeByCode = new Map(
  phoneCountryCodes.map((country) => [country.code, country])
);
