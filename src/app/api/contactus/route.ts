...
const CONTACT_ACKNOWLEDGEMENT_SUBJECT = 'Thanks for Reaching Out!';
...
function getContactAcknowledgementEmailText() {
  return [
    'Thanks for Reaching Out!',
    '',
    'Hello,',
    '',
    "Thank you for getting in touch! We've received your enquiry and a member of our team will respond during our business hours, Monday to Friday, 8:00 AM - 5:00 PM.",
    '',
...
function getContactAcknowledgementEmailHtml() {
  const contentHtml = `
  ...
  return `
    ...
    <title>CMS Prime - Contact Confirmation</title>
    <style>
      body {
        margin: 0;
        padding: 0;
...
async function sendContactAcknowledgementEmail(recipientEmail: string) {
  if (!SENDGRID_API_KEY) {
  ...
  const response = await fetch(SENDGRID_API_URL, {
    ...
    body: JSON.stringify({
      ...
      content: [
        {
          type: 'text/plain',
          value: getContactAcknowledgementEmailText(),
        },
        {
          type: 'text/html',
          value: getContactAcknowledgementEmailHtml(),
        },
      ],
    }),
  });
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      await sendContactAcknowledgementEmail(parsedBody.data.email);
    } catch (emailError) {
      emailSent = false;
      warnings.push(
        'Contact user was created, but the acknowledgement email could not be delivered.'
      );
      console.error('Contact acknowledgement email failed:', emailError);
    }
