...
const CPAY_EMAIL_SUBJECT = 'Your CPAY Trading Mastercard Request Was Received';
...
function getCpayEmailText() {
  return [
    'Hello,',
    '',
    'Thank you for your request for a CPAY Trading Mastercard. A member of our team will be in touch with you shortly.',
    '',
    "Please note that this card is designed specifically for clients with an active trading account, offering a seamless way to manage deposits and withdrawals. If you do not yet have a trading account with us but are an active trader, we'll be happy to help you get started so you can enjoy all the benefits of CPAY.",
    '',
    'Best regards,',
    'CMS Prime',
  ].join('\n');
...
function getCpayEmailHtml() {
  ...
  return `
    ...
    <title>CMS Prime - CPAY Registration</title>
    <style>
      body {
        margin: 0;
        padding: 0;
...
function remapCpayRegistrationErrors(errors: unknown) {
  ...
  if (children.firstName && !children.fullName) {
    children.fullName = children.firstName;
...
async function sendCpayRegistrationEmail(recipientEmail: string) {
  if (!SENDGRID_API_KEY) {
  ...
  const response = await fetch(SENDGRID_API_URL, {
    ...
    body: JSON.stringify({
      ...
      content: [
        {
          type: 'text/plain',
          value: getCpayEmailText(),
        },
        {
          type: 'text/html',
          value: getCpayEmailHtml(),
        },
      ],
    }),
  });
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('=== CPAY REGISTRATION DEBUG ===');
    console.log('Request body:', JSON.stringify(body, null, 2));

    const parsedBody = cpayRegistrationSchema.safeParse(body);
    ...
    const payload = {
      firstName: parsedBody.data.fullName,
    ...
    if (!response.ok) {
      const failureResponse: {
        ...
        message: data.message || 'CPAY registration failed',
        errors: remapCpayRegistrationErrors(data.errors),
      };
    ...
    try {
      await sendCpayRegistrationEmail(parsedBody.data.email);
    } catch (emailError) {
      emailSent = false;
      warning =
        'CPAY registration was submitted, but the confirmation email could not be delivered.';
      console.error('CPAY registration email failed:', emailError);
    }
